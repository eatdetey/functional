...
razm_no_repeat(_, 0, Razm, Razm) :- !.
razm_no_repeat(Alphabet, NCur, RazmCur, Razm) :-
    my_select(El, Alphabet, RestAlphabet),
    NNew is NCur - 1,
    razm_no_repeat(RestAlphabet, NNew, [El|RazmCur], Razm).

my_select(El, [El|T], T).
my_select(El, [H|T], [H|R]):-my_select(El, T, R).
...
sochetP([],_,0):-!.
sochetP([H|Sub_set],[H|Set],K):-
	K1 is K-1, 
	sochetP(Sub_set,[H|Set],K1).
sochetP(Sub_set,[H|Set],K):-
	sochetP(Sub_set,Set,K).
